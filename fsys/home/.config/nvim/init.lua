...
-- go easy on the eyes
require("catppuccin").setup({ transparent_background = true, flavour = "mocha" })
...
require("mason-lspconfig").setup({ handlers = { function(sn) require('lspconfig')[sn].setup({ capabilities = capabilities }) end } })
...
vim.keymap.set("n", "<leader>sn", function()
  tlsc_builtin.find_files({ cwd = vim.fn.stdpath("config") })
...
cmp.setup({
  ...
  mapping = cmp.mapping.preset.insert({
    ...
    ["<C-l>"] = cmp.mapping(function() if luasnip.expand_or_locally_jumpable() then luasnip.expand_or_jump() end end,
      { "i", "s" }),
...
vim.diagnostic.config({ float = winconf })
